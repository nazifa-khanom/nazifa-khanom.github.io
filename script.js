...
function applySectionCoverState(d,key){
  ...
  root.style.setProperty("--cutout-zoom-scale",String(zoom/100));
  ...
  if(enabled){
    ...
    sidebar?.querySelector(".section-cover-photo-layer")?.remove();
  }

  if(sidebar){
    const hasPhoto=enabled&&!!String(d.photo_url||"").trim();
    ...
    if(!enabled){
      sidebar.classList.remove("has-section-cover-photo");
    }
  }
}
...
let projectMasonryRaf=0;
let projectMasonryWidth=0;
let projectMasonryResizeObserver=null;
let projectMasonryMediaEventsBound=false;

function clearProjectMasonry(){
  const grid=$("projectsList");
  if(!grid)return;
  grid.classList.remove("project-flow-masonry");
...
function layoutProjectMasonry(){
  ...
  const grid=$("projectsList");
  if(!grid)return;
  const mode=document.documentElement.dataset.projectFlow||"grid";
  const columns=Math.max(1,Math.min(3,Number(getComputedStyle(document.documentElement).getPropertyValue("--site-project-columns"))||1));
  const mobile=window.matchMedia("(max-width: 900px)").matches;
  const cards=[...grid.children].filter(el=>el.classList.contains("card"));

  if(mode!=="masonry"||columns===1||mobile||!cards.length||grid.clientWidth<1){
    clearProjectMasonry();
    return;
  }

  grid.classList.add("project-flow-masonry");
  grid.style.setProperty("display","block","important");
  grid.style.setProperty("position","relative","important");

  const computed=getComputedStyle(grid);
  const gap=parseFloat(computed.columnGap||computed.gap)||14;
  const width=Math.max(1,(grid.clientWidth-gap*(columns-1))/columns);
  ...
  cards.forEach((card,index)=>{
    ...
    card.style.setProperty("left",`${col*(width+gap)}px`,"important");
    card.style.setProperty("top",`${heights[col]}px`,"important");
...
function scheduleProjectMasonry(){
  if(projectMasonryRaf)cancelAnimationFrame(projectMasonryRaf);
  projectMasonryRaf=requestAnimationFrame(()=>requestAnimationFrame(layoutProjectMasonry));
}

function setupProjectMasonryWatchers(){
  const grid=$("projectsList");
  if(!grid)return;
  if(projectMasonryResizeObserver)projectMasonryResizeObserver.disconnect();
  projectMasonryResizeObserver=new ResizeObserver(entries=>{
    const width=Math.round(entries[0]?.contentRect?.width||0);
    if(width!==projectMasonryWidth){
      projectMasonryWidth=width;
      scheduleProjectMasonry();
    }
  });
  projectMasonryResizeObserver.observe(grid);
  if(!projectMasonryMediaEventsBound){
    grid.addEventListener("load",scheduleProjectMasonry,true);
    grid.addEventListener("loadedmetadata",scheduleProjectMasonry,true);
    projectMasonryMediaEventsBound=true;
  }
}
...
function applySiteSettings(d){
  ...
  setupProjectMasonryWatchers();
  scheduleProjectMasonry();
  setTimeout(scheduleProjectMasonry,180);
  setTimeout(scheduleProjectMasonry,700);
}
...
const EDUCATION_DOCUMENT_PRESENTATIONS={
  "provisional-certificate":{defaultStyle:"official-credential",kicker:"Academic credential",buttonLabel:"View Provisional Certificate",fallbackLabel:"Provisional Certificate",styles:["official-credential","framed-certificate","split-dossier","gallery-display","minimal-archive"]},
  "transcript":{defaultStyle:"official-transcript",kicker:"Academic record",buttonLabel:"View Transcript",fallbackLabel:"Transcript",styles:["official-transcript","ledger-record","academic-dossier","registry-sheet","minimal-record"]},
  "testimonial":{defaultStyle:"institutional-letter",kicker:"Institutional endorsement",buttonLabel:"View Testimonial",fallbackLabel:"Testimonial",styles:["institutional-letter","endorsement-frame","signature-focus","reference-dossier","minimal-letter"]}
};
...
function normalizeEducationDocumentPresentation(src,id){
  ...
  return {
    presentationStyle:config.styles.includes(item.presentationStyle)?item.presentationStyle:config.defaultStyle,
...
function normalizeEducationDocuments(content){
  ...
  const normalized=raw.map((item,index)=>{
    ...
    return {id,title:String(src.title||"").trim(),description:String(src.description||""),visible:src.visible!==false,url:String(src.url||""),filename:String(src.filename||""),updated_at:String(src.updated_at||""),...normalizeEducationDocumentPresentation(src,id)};
  });
  const used=new Set();
  const permanent=PERMANENT_EDUCATION_DOCUMENTS.map(def=>{
    ...
    return {id:def.id,title:def.title,description:"",visible:true,url:"",filename:"",updated_at:"",...normalizeEducationDocumentPresentation({},def.id)};
  });
...
function featuredEducationDocumentPanelHtml(doc){
  const config=educationDocumentPresentationConfig(doc?.id);
  if(!config)return "";
  const view=educationDocumentViewId(doc),url=safeUrl(doc.url),presentation=normalizeEducationDocumentPresentation(doc,doc.id);
  ...
  const title=doc.title||config.fallbackLabel,kind=String(doc.id||"document");
  const downloadName=doc.filename||`${title.replace(/\s+/g,"_")}.pdf`;
  const previewHtml=url?`<div class="education-featured-document-preview provisional-certificate-preview provisional-preview-size-${escAttr(size)} provisional-preview-fit-${escAttr(fit)}">
    <a class="education-featured-document-page provisional-certificate-page" href="${escAttr(url)}" target="_blank" rel="noopener" data-lightbox-media-kind="pdf" data-lightbox-mode="on" data-media-url="${escAttr(url)}" data-media-preview="${escAttr(preview||"")}" data-media-title="${escAttr(title)}" data-media-caption="${escAttr(doc.description||"")}" data-media-download-name="${escAttr(downloadName)}">
      ${preview?`<img src="${escAttr(preview)}" alt="Preview of ${escAttr(title)}" loading="lazy">`:`<div class="provisional-certificate-pdf-fallback education-featured-document-fallback"><span>PDF</span><strong>${esc(config.fallbackLabel)}</strong><small>Open the document to view it.</small></div>`}
    </a>
  </div>`:"";
  return `<div class="education-document-panel provisional-certificate-panel education-featured-document-panel education-featured-document-${escAttr(kind)} hidden" data-education-panel="${escAttr(view)}" data-education-dynamic="true" data-document-kind="${escAttr(kind)}" data-document-style="${escAttr(style)}" data-provisional-style="${escAttr(style)}">
    <article class="provisional-certificate-layout education-featured-document-layout">
      <div class="provisional-certificate-copy education-featured-document-copy"><span class="gradesheet-kicker">${esc(config.kicker)}</span><h3>${esc(title)}</h3>${doc.description?`<p class="muted">${esc(doc.description)}</p>`:""}${doc.filename?`<p class="education-document-meta">${esc(doc.filename)}${doc.updated_at?` · updated ${esc(formatDate(doc.updated_at))}`:""}</p>`:""}<div class="provisional-certificate-actions education-featured-document-actions">${url?`<a class="button" href="${escAttr(url)}" target="_blank" rel="noopener">${esc(config.buttonLabel)}</a>`:`<span class="muted small">Document has not been uploaded yet.</span>`}</div></div>
      ${previewHtml}
    </article>
  </div>`;
}
function educationDocumentPanelHtml(doc){
  if(educationDocumentPresentationConfig(doc?.id))return featuredEducationDocumentPanelHtml(doc);
...
function courseRecordSettings(content){
  ...
  const styleCandidate=raw.style||legacy.style||"academic-ledger";
  return {
    style:GRADESHEET_STYLE_VALUES.includes(styleCandidate)?styleCandidate:"academic-ledger",
...
function repairPreGradesheetStateInMemory(content){
  if(!content||typeof content!=="object")return content;
  const settings=courseRecordSettings(content);
  content.builderState=(content.builderState&&typeof content.builderState==="object")?content.builderState:{};
  if(content.builderState.coursesRepairV3!==true){
    // Restore only the builder/site settings from the user's last backup before
    // the gradesheet feature. Personal content remains the newest saved content.
    content.siteSettings=structuredClone(PRE_GRADE_SITE_SETTINGS);
    if(Array.isArray(content.builderState.revisions))content.builderState.revisions=content.builderState.revisions.filter(r=>!legacyCourseRevision(r));
    content.builderState.coursesRepairV3=true;
  }
  content.courseRecord=settings;
  if(content.gradesheet)delete content.gradesheet;
  return content;
}
...
function setEducationLocalView(view,{scroll=false}={}){
  ...
  if(next==="gradesheet"&&currentRenderedContent)renderGradesheetLazy(currentRenderedContent);
  if(scroll){
...
function bindEducationLocalNav(){
  ...
  document.addEventListener("click",event=>{
    ...
    setEducationLocalView(button.dataset.educationView,{scroll:true});
  });
}

function courseGradePanelHtml(settings){
  const g=ACADEMIC_GRADE_DATA,final=g.final||{};
  return `<div class="education-courses-heading"><div><span class="gradesheet-kicker">Official academic record</span><h3>${esc(g.title||"Undergraduate Gradesheet")}</h3><p class="muted small">${esc(g.subtitle||"Complete undergraduate academic record")}</p></div></div>
...
function prepareGradesheetPanel(d){
  ...
  if(!panel)return;
  const settings=courseRecordSettings(d);
  panel.dataset.gradesheetStyle=settings.style;
  gradesheetRendered=false;
  const mount=$("gradesheetMount");
  if(mount)mount.innerHTML='<div class="gradesheet-loading-placeholder"><span>Gradesheet</span><small>Select Gradesheet to load the semester record.</small></div>';
}

function renderGradesheetLazy(d){
  const panel=$("educationCoursesPanel"),mount=$("gradesheetMount");
  if(!panel||!mount)return;
  const settings=courseRecordSettings(d);
  panel.dataset.gradesheetStyle=settings.style;
  if(gradesheetRendered&&mount.dataset.style===settings.style&&mount.dataset.url===settings.url)return;
  mount.innerHTML=courseGradePanelHtml(settings);
  mount.dataset.style=settings.style;
  mount.dataset.url=settings.url;
  gradesheetRendered=true;
}
...
function render(d){
  ...
  if(educationTabs){
    ...
    educationTabs.dataset.activityTabStyle=educationTabStyle;
  }
  bindEducationLocalNav();
  setEducationLocalView(educationLocalView);
  ...
  applySiteSettings(d);
  // On a reload, the remembered Gradesheet sub-view is restored before
  ...
  if(educationLocalView==="gradesheet")renderGradesheetLazy(d);
}
